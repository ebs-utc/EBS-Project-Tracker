...
function AuthProvider({ children }) {
  ...
  return <AuthCtx.Provider value={{ user, loading, signIn, signOut, isAdmin: !!user }}>{children}</AuthCtx.Provider>
}
...
function AdminUsersPage() {
  ...
  const handleCreateUser = async () => {
    setError(''); setMessage('')
    try {
      const { error: err } = await supabase.auth.signUp({ email: newEmail, password: newPassword })
      if (err) throw err
      setMessage(`User ${newEmail} created successfully!`); setNewEmail(''); setNewPassword(''); setShowCreate(false)
  ...
  return <div className="max-w-2xl mx-auto">
    <div className="mb-8"><h1 className="text-2xl font-bold font-display text-surface-900">User Management</h1><p className="text-sm text-surface-500 mt-1">Create and manage admin users</p></div>
      ...
      {showCreate && (
        <div className="space-y-4 pt-4 border-t border-surface-100">
          ...
          <div className="flex gap-3 justify-end">
            ...
            <button onClick={handleCreateUser} disabled={!newEmail || !newPassword || newPassword.length < 6} className="px-4 py-2 rounded-lg bg-brand-600 text-white hover:bg-brand-700 font-medium text-sm disabled:opacity-40 transition-colors">Create User</button>
          </div>
        </div>
      )}
    </div>
